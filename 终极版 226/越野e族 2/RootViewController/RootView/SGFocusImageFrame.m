...
- (void)setupViewsWithType:(int)type;
...
- (id)initWithFrame:(CGRect)frame delegate:(id<SGFocusImageFrameDelegate>)delegate imageItems:(NSArray *)items isAuto:(BOOL)isAuto type:(int)sgtype
{
    ...
    if (self)
    {
        ...
        objc_setAssociatedObject(self, (const void *)SG_FOCUS_ITEM_ASS_KEY, imageItems, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        _isAutoPlay = YES;
        [self setupViewsWithType:sgtype];
...
{
    return [self initWithFrame:frame delegate:delegate imageItems:items isAuto:YES type:1];
}
...
- (void)setupViewsWithType:(int)type
{
    ...
    if (type==2) {
        ...
        CGSize size = CGSizeMake(320, 0);
        //pagecontrol
        _pagecontrol = [[SMPageControl alloc]initWithFrame:CGRectMake(130, 0,  320-255, 25)];
        
        _pagecontrol.backgroundColor = [UIColor clearColor];
        _pagecontrol.numberOfPages =imageItems.count>=2?imageItems.count-2:0 ;
        _pagecontrol.indicatorMargin=6.0f;
        [_pagecontrol setPageIndicatorImage:[UIImage imageNamed:@"ZMallblackround9x9.png"]];
        [_pagecontrol setCurrentPageIndicatorImage:[UIImage imageNamed:@"ZMallwitheground9x9.png"]];
